...
-- И очень полезная sp_WhoIsActive (не стандартная, надо скачать и установить)
...
-- Status:
-- * Running    - выполняется
-- * Background - работает в фоне (например, обнаружение дедлоков)
-- * Suspended  - ожидает ресурса (например, ввода-вывода)
-- * Runnable   - готов выполняться, но ждет, когда потоку дадут процессорное время
-- * Pending    - готов выполняться, но ждет рабочих потоков 
-- * Rollback   - выполняется откат транзакции
-- * Dormant    - сброс соединения
-- * Spinloop   - ожидание spinlock (внутренних блокировок SQL Server)
-- * Sleeping   - нет работы
